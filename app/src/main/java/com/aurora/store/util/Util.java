...
public class Util {
    ...
    public static int parseFloatToInt(Float val) {
        try {
            return (int) (val * 10);
        } catch (NumberFormatException e) {
            return -1;
    ...
    public static void clearCache(Context context) {
        PrefUtil.putString(context, Constants.TOP_APPS, "");
        PrefUtil.putString(context, Constants.TOP_FAMILY, "");
        PrefUtil.putString(context, Constants.TOP_GAME, "");
    }

    public static void restartApp(Context context) {
        Intent activity = new Intent(context, SplashActivity.class);
        int id = 1337;
        PendingIntent intent = PendingIntent.getActivity(context, id, activity,
                PendingIntent.FLAG_CANCEL_CURRENT);
        AlarmManager alarmManager = (AlarmManager) context.getSystemService(Context.ALARM_SERVICE);
        if (alarmManager != null)
            alarmManager.set(AlarmManager.RTC, System.currentTimeMillis() + 100, intent);
        System.exit(0);
    }
